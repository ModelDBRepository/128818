...
% load-up nspks from NMDA results
load('NMDA_NEW_single_trial_bimodality_test_D1_0','nspks')
NMDAspks = nspks;
...
for j = 1:numel(mNMDA)
    ...
    % PSPampa = Xsyn * mNMDA(j)/ampa_nmda;    % control#1: increase AMPA to
    % same conductance
    ...
    for loop = 1:nHz
        ...
        for i = 1:n-1
            ...
            BD1all_nmda = 1;    % control#2 -switch off gate...
...
figure(5), clf;
plot(mNMDA,nspks); hold on;
plot(mNMDA,NMDAspks,':')


fname = ['NMDA_nogate_single_trial_bimodality_test_D1_' num2str(D1) '.mat'];
